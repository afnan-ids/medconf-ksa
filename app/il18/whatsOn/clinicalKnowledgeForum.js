export const clinicalKnowledgeForumTranslations = {
  en: {
    home: "Home",
    whatsOn: "whats on",

    cmeAccredited: "CME Accredited 2026 Edition",
    clinicalKnowledgeForum: "Clinical knowledge forum",
    heroDescription:
      "Sessions where doctors and experts discuss medical research, treatment methods, and healthcare advancements.",

    advancingClinicalExcellence:
      "Advancing Clinical Excellence Through Knowledge Sharing",

    heroSubDescription:
      "Join 500+ healthcare professionals for 30+ expert-led sessions covering the latest breakthroughs in medical research, innovative treatment protocols, and emerging healthcare technologies. Earn CME credits while learning from world-renowned clinicians and researchers.",

    cmeCredits: "CME Credits",
    expertSpeakers: "Expert Speakers",
    researchPresentations: "Research Presentations",
    networking: "Networking",

    allSessions: "All Sessions",
    keynotes: "Keynotes",
    panelDiscussions: "Panel Discussions",
    workshops: "Workshops",
    caseStudies: "Case Studies",

    conferenceSchedule: "Conference Schedule",
    eventAgenda: "Event Agenda",
    scheduleDescription:
      "2 days of groundbreaking clinical insights and networking",

    day1: "Day 1-May 15,2026",
    day2: "Day 2-May 16,2026",

    mainHallA: "Main Hall A",
    openingKeynote:
      "Opening Keynote: The Future of Precision Medicine",
    drSarahSession:
      "Dr. Sarah Johnson • Chief of Oncology, Mayo Clinic",
    keynote: "Keynote",

    aiRadiology:
      "AI in Radiology: Transforming Diagnostics",
    drMichaelSession:
      "Dr. Michael Chen • Director of Medical Imaging, Stanford Health",

    hallB: "Hall B",
    cardiovascularPanel:
      "Panel: Cardiovascular Innovations",
    cardiovascularPanelDesc:
      "Panel Discussions.4 leading Cardiologists",
    panel: "panel",

    workshopRoom1: "Workshop Room 1",
    surgicalWorkshop:
      "Workshop: Minimally Invasive Surgical Techniques",
    drRobertSession:
      "Dr.Robert williams.Head of Robotics Surgery,johns Hopkins",
    workshop: "Workshop",

    lunchNetworking:
      "Lunch Break & Networking",
    exhibitionHall: "Exhibition hall",

    hallC: "Hall C",
    rareDiseaseCaseStudy:
      "Case Study: Rare Disease Diagnosis",
    drEmilySession:
      "Dr.Emily Rodriguez.Clinical Geneticist,Boston Childernns's",
    caseStudy: "Case-study",

    mentalHealthPanel:
      "Panel: Mental Health in the Digital Age",
    mentalHealthPanelDesc:
      "Panel Discussion • 5 Psychiatry Experts",

    closingSession:
      "Closing Session: Future of Global Healthcare",
    drJamesSession:
      "Dr. James Wilson • WHO Director of Research",

    downloadAgenda:
      "Download Full Agenda (PDF)",

    worldRenownedFaculty:
      "World-Renowned Faculty",
    featuredSpeakers:
      "Featured Speakers",
    facultyDescription:
      "Learn from leading clinicians, researchers, and healthcare innovators",

    drSarahJohnson:
      "Dr. Sarah Johnson",
    chiefOncology:
      "Chief of Oncology, Mayo Clinic",
    speakingPrecisionMedicine:
      "Speaking on: Precision Medicine",

    drMichaelChen:
      "Dr. Michael Chen",
    directorImaging:
      "Director of Imaging, Stanford Health",
    speakingAIRadiology:
      "Speaking on: AI in Radiology",

    drRobertWilliams:
      "Dr. Robert Williams",
    headRoboticSurgery:
      "Head of Robotic Surgery, Johns Hopkins",
    speakingSurgicalInnovation:
      "Speaking on: Surgical Innovation",

    drEmilyRodriguez:
      "Dr. Emily Rodriguez",
    clinicalGeneticist:
      "Clinical Geneticist, Boston Children's",
    speakingRareDiseases:
      "Speaking on: Rare Diseases",

    drJamesWilson:
      "Dr. James Wilson",
    whoResearchDirector:
      "WHO Director of Research",
    speakingGlobalHealth:
      "Speaking on: Global Health",

    drLisaPatel:
      "Dr. Lisa Patel",
    chiefImmunotherapy:
      "Chief of Immunotherapy, MD Anderson",
    speakingCancerResearch:
      "Speaking on: Cancer Research",

    research: "Research",
    keyFindingsPresented:
      "Key Findings Presented",
    researchDescription:
      "Groundbreaking research being shared at this year's forum",

    earlyCancerDetection:
      "Breakthrough in Early Cancer Detection",
    oncology: "Oncology",
    earlyCancerDetectionDesc:
      "New liquid biopsy technique shows 94% accuracy in detecting 12 cancer types at stage 1",

    aiHeartDisease:
      "AI Predicts Heart Disease 5 Years in Advance",
    cardiology: "Cardiology",
    aiHeartDiseaseDesc:
      "Machine learning model analyzes ECG data to predict cardiovascular events with 87% accuracy.",

    sickleCellTreatment:
      "Gene Editing Success in Sickle Cell Treatment",
    genetics: "Genetics",
    sickleCellTreatmentDesc:
      "CRISPR-based therapy shows promising results in 95% of clinical trial participants.",

    telemedicineER:
      "Telemedicine Reduces ER Visits by 40%",
    digitalHealth: "Digital Health",
    telemedicineERDesc:
      "Remote monitoring program for clinical patients significantly reduces hospital admissions.",

    forumImpact: "Forum Impact",
    knowledgeExchange:
      "Knowledge Exchange",
    forumImpactDesc:
      "2 days of transformative clinical education",

    expertSessions:
      "Expert Sessions",
    renownedSpeakers:
      "Renowned Speakers",
    cmeCreditsAvailable:
      "CME Credits Available",
    healthcareProfessionals:
      "Healthcare Professionals",

    reserveYourSeat:
      "Reserve Your Seat",
    reserveDescription:
      "Join 500+ healthcare professionals for 2 days of cutting-edge clinical education, networking, and CME credits",

    registerNow: "Register Now",
    viewAgenda: "View Agenda",
  },

  ar: {
    home: "الرئيسيه",
    whatsOn: "ما هو المعروض",

    cmeAccredited: "ساعات تدريبية معتمدة",
    clinicalKnowledgeForum: "منتدى المعرفة السريرية",
    heroDescription:
      "جلسات يناقش فيها الأطباء والخبراء الأبحاث الطبية وأساليب العلاج والتطورات في مجال الرعاية الصحية.",

    advancingClinicalExcellence:
      "تعزيز التميز السريري من خلال تبادل المعرفة",

    heroSubDescription:
      "انضم إلى أكثر من 500 متخصص في الرعاية الصحية لحضور أكثر من 30 جلسة يقودها خبراء، تغطي أحدث الإنجازات في البحوث الطبية، وبروتوكولات العلاج المبتكرة، وتقنيات الرعاية الصحية الناشئة. احصل على ساعات معتمدة للتعليم الطبي المستمر بينما تتعلم من أطباء وباحثين عالميين مرموقين.",

    cmeCredits: "ساعات تدريبية معتمدة",
    expertSpeakers: "متحدثون خبراء",
    researchPresentations: "عروض الأبحاث",
    networking: "العلاقات",

    allSessions: "جميع الفصول",
    keynotes: "الكلمات الرئيسية",
    panelDiscussions: "حلقات نقاش",
    workshops: "ورش العمل",
    caseStudies: "دراسات الحالة",

    conferenceSchedule: "جدول المؤتمر",
    eventAgenda: "أجندة المعرض",
    scheduleDescription:
      "يومان من الرؤى السريرية الرائدة والتواصل",

    day1: "اليوم الاول",
    day2: "اليوم الثاني",

    mainHallA: "القاعة الرئيسية",
    openingKeynote:
      "الكلمة الافتتاحية: مستقبل الطب الدقيق",
    drSarahSession:
      "الدكتورة سارة جونسون • رئيسة قسم الأورام، مايو كلينك",
    keynote: "الكلمة الرئيسية",

    aiRadiology:
      "الذكاء الاصطناعي في الأشعة: تحويل التشخيص",
    drMichaelSession:
      "الدكتور مايكل تشين • مدير قسم التصوير الطبي، ستانفورد هيلث",

    hallB: "القاعة ب",
    cardiovascularPanel:
      "حلقة نقاش: ابتكارات أمراض القلب والأوعية الدموية",
    cardiovascularPanelDesc:
      "حلقات نقاش. أربعة من كبار أطباء القلب",
    panel: "لوحة",

    workshopRoom1:
      "ورشة عمل غرفة 1",
    surgicalWorkshop:
      "ورشة عمل: التقنيات الجراحية طفيفة التوغل",
    drRobertSession:
      "الدكتور روبرت ويليامز، رئيس قسم جراحة الروبوتات، جامعة جونز هوبكنز",
    workshop: "ورشة عمل",

    lunchNetworking:
      "استراحة الغداء والتواصل",
    exhibitionHall:
      "قاعة المعارض",

    hallC: "القاعة ج",
    rareDiseaseCaseStudy:
      "دراسة حالة: تشخيص الأمراض النادرة",
    drEmilySession:
      "الدكتورة إميلي رودريغيز، أخصائية علم الوراثة السريرية، مستشفى بوسطن للأطفال",
    caseStudy: "دراسة حالة",

    mentalHealthPanel:
      "حلقة نقاش: الصحة النفسية في العصر الرقمي",
    mentalHealthPanelDesc:
      "حلقة نقاش • 5 خبراء في الطب النفسي",

    closingSession:
      "الجلسة الختامية: مستقبل الرعاية الصحية العالمية",
    drJamesSession:
      "الدكتور جيمس ويلسون • مدير الأبحاث في منظمة الصحة العالمية",

    downloadAgenda:
      "حمل الاجندة كاملة",

    worldRenownedFaculty:
      "أعضاء هيئة تدريس ذوو شهرة عالمية",
    featuredSpeakers:
      "المتحدثون المميزون",
    facultyDescription:
      "تعلم من كبار الأطباء والباحثين والمبتكرين في مجال الرعاية الصحية",

    drSarahJohnson:
      "الدكتورة ساره جونسون",
    chiefOncology:
      "رئيس قسم الأورام، مايو كلينك",
    speakingPrecisionMedicine:
      "الحديث عن: الطب الدقيق",

    drMichaelChen:
      "الدكتور ميخائئل تشين",
    directorImaging:
      "مدير قسم التصوير الطبي، ستانفورد هيلث",
    speakingAIRadiology:
      "الحديث عن: الذكاء الاصطناعي في علم الأشعة",

    drRobertWilliams:
      "الدكتور روبرت وليام",
    headRoboticSurgery:
      "رئيس قسم الجراحة الروبوتية، جامعة جونز هوبكنز",
    speakingSurgicalInnovation:
      "التحدث عن إبتكارات الجراحة",

    drEmilyRodriguez:
      "الدكتور إيميلي رودريقوز",
    clinicalGeneticist:
      "أخصائي علم الوراثة السريرية، مستشفى بوسطن للأطفال",
    speakingRareDiseases:
      "الحديث عن: الأمراض النادرة",

    drJamesWilson:
      "الدكتور جيمس ويلسون",
    whoResearchDirector:
      "مدير البحوث في منظمة الصحة العالمية",
    speakingGlobalHealth:
      "التحدث عن الصحة العالمية",

    drLisaPatel:
      "الدكتورة ليزا باتيل",
    chiefImmunotherapy:
      "رئيس قسم العلاج المناعي، مركز إم دي أندرسون للسرطان",
    speakingCancerResearch:
      "التحدث عن ابحاث السرطان",

    research: "بحث",
    keyFindingsPresented:
      "النتائج الرئيسية المقدمة",
    researchDescription:
      "سيتم عرض أبحاث رائدة في منتدى هذا العام",

    earlyCancerDetection:
      "إنجازٌ هام في الكشف المبكر عن السرطان",
    oncology: "الاورام",
    earlyCancerDetectionDesc:
      "أظهرت تقنية الخزعة السائلة الجديدة دقة بنسبة 94% في الكشف عن 12 نوعًا من السرطان في المرحلة الأولى.",

    aiHeartDisease:
      "الذكاء الاصطناعي يتنبأ بأمراض القلب قبل 5 سنوات",
    cardiology: "القلب",
    aiHeartDiseaseDesc:
      "يقوم نموذج التعلم الآلي بتحليل بيانات تخطيط كهربية القلب للتنبؤ بأحداث القلب والأوعية الدموية بدقة 87%.",

    sickleCellTreatment:
      "نجاح تعديل الجينات في علاج فقر الدم المنجلي",
    genetics: "الجينات",
    sickleCellTreatmentDesc:
      "أظهر العلاج القائم على تقنية كريسبر نتائج واعدة في 95% من المشاركين في التجارب السريرية.",

    telemedicineER:
      "التطبيب عن بعد يقلل زيارات الطوارئ بنسبة 40%",
    digitalHealth: "الصحة الرقميه",
    telemedicineERDesc:
      "برنامج المراقبة عن بعد للمرضى السريريين يقلل بشكل كبير من حالات دخول المستشفى.",

    forumImpact: "تأثير المنتدى",
    knowledgeExchange:
      "تبادل المعرفة",
    forumImpactDesc:
      "يومين من التعليم السريري التحويلي",

    expertSessions:
      "جلسات الخبراء",
    renownedSpeakers:
      "المتحدثون المشهورون",
    cmeCreditsAvailable:
      "تتوفر ساعات معتمدة للتعليم الطبي المستمر",
    healthcareProfessionals:
      "أخصائيو الرعاية الصحية",

    reserveYourSeat:
      "احجز مقعدك",
    reserveDescription:
      "انضم إلى أكثر من 500 متخصص في الرعاية الصحية لمدة يومين من التعليم السريري المتطور، والتواصل، واحصل على ساعات معتمدة للتعليم الطبي المستمر.",

    registerNow: "سجل الان",
    viewAgenda: "الاطلاع على الاجندة",
  },
};