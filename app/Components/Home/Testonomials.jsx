          ...
          <div className="p-4 sm:p-8 lg:p-12 max-w-7xl mx-auto relative">
            ...
            <div className="relative z-10 max-w-5xl mx-auto">
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${current.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}></div>
                ...
                <div className="relative mb-6 sm:mb-8">
                  <div className={`absolute inset-0 bg-gradient-to-r ${current.color} rounded-xl sm:rounded-2xl blur-md sm:blur-lg opacity-20 group-hover:opacity-40 transition-opacity duration-500`}></div>
                  ...
                  {/* Role Badge */}
                  <div className={`absolute -top-3 -right-3 sm:-top-4 sm:-right-4 bg-gradient-to-r ${current.color} rounded-full px-3 py-1`}>
                    <span className="text-[10px] sm:text-xs font-bold text-white">{current.role}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-6 sm:mb-8">
                  <div className="flex gap-1">
                    {[...Array(current.rating)].map((_, idx) => (
                      <Star key={idx} className="w-4 h-4 sm:w-5 sm:h-5 fill-yellow-400 text-yellow-400" />
                    ))}
                  </div>
                  <div className={`w-px h-6 bg-gradient-to-b ${current.color}`}></div>
                  <div className={`text-xs sm:text-sm text-transparent bg-clip-text bg-gradient-to-r ${current.color} font-medium`}>
                    {current.yearsAttended} attending
                  </div>
                </div>

                {/* Author Divider */}
                <div className={`w-16 h-0.5 bg-gradient-to-r ${current.color} mb-5 sm:mb-6`}></div>

                {/* Author Info */}
                <div>
                  <p className="text-lg sm:text-xl font-bold text-white">{current.name}</p>
              ...
              <div className="flex justify-center gap-4 mt-8 sm:mt-10">
                ...
                <div className="flex gap-2 items-center">
                  {testimonials.map((t, idx) => (
                    ...
                    >
                      {activeIndex === idx && (
                        <span className={`absolute -top-4 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-gradient-to-r ${t.color}`}></span>
                      )}
                    </button>
                  ))}
                </div>
    ...
    {testimonials.map((t, idx) => {
      ...
      return (
        ...
        >
          ...
          <div className={`absolute inset-0 bg-gradient-to-r ${t.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}></div>
          
          {/* Active glow effect - subtle */}
          {isActive && (
            <div className={`absolute inset-0 bg-gradient-to-r ${t.color} opacity-5 rounded-xl sm:rounded-2xl`}></div>
          )}
          
          <div className="flex items-center gap-2 mb-2 relative z-10">
            <div className={`w-6 h-6 rounded-full bg-gradient-to-r ${t.color} flex items-center justify-center ${isActive ? "ring-1 ring-white/30" : ""}`}>
          ...
          <p className={`text-xs line-clamp-2 transition-colors duration-300 relative z-10 ${
            isActive ? "text-gray-200" : "text-gray-400 group-hover:text-gray-300"
          }`}>
            "{t.quote.slice(0, 55)}..."
          </p>
          
          <p className={`text-[10px] mt-2 transition-colors duration-300 relative z-10 ${
          ...
          {isActive && (
            <div className={`absolute bottom-0 left-3 right-3 h-0.5 bg-gradient-to-r ${t.color} rounded-full`}></div>
          )}
        </button>
      );
    })}
