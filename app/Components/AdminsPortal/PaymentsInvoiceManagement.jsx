...
//       items: [
//         { description: "Booth Rental - A12", quantity: 1, unitPrice: 3500, total: 3500 },
//         { description: "Premium Package Upgrade", quantity: 1, unitPrice: 1500, total: 1500 },
//       ],
...
//       items: [
//         { description: "Booth Rental - C21", quantity: 1, unitPrice: 5500, total: 5500 },
//         { description: "Corner Package Upgrade", quantity: 1, unitPrice: 2000, total: 2000 },
//       ],
...
//       items: [
//         { description: "Platinum Plus Sponsorship", quantity: 1, unitPrice: 150000, total: 150000 },
//       ],
//       paymentDate: null,
//       transactionId: null,
//     },
...
//       items: [
//         { description: "Booth Rental - B07", quantity: 1, unitPrice: 2500, total: 2500 },
//         { description: "Standard Package", quantity: 1, unitPrice: 1000, total: 1000 },
//       ],
//       paymentDate: null,
//       transactionId: null,
//     },
...
//       items: [
//         { description: "Platinum Premier Sponsorship", quantity: 1, unitPrice: 200000, total: 200000 },
//       ],
...
//     { value: "completed", label: "Completed", color: "emerald", icon: CheckCircle },
//   ];
...
//     const matchesSearch = t.customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
//     paidCount: transactions.filter(t => t.status === "paid" || t.status === "completed").length,
...
//       t.id === id ? { ...t, status: newStatus, paymentDate: newStatus === "paid" ? new Date().toISOString().split("T")[0] : t.paymentDate } : t
//     ));
//   };
...
//         {[
//           { label: "Total Revenue", value: `$${(stats.totalRevenue / 1000).toFixed(0)}K`, icon: DollarSign, color: "emerald", trend: "+18%", trendUp: true },
//           { label: "Pending Amount", value: `$${(stats.pendingAmount / 1000).toFixed(0)}K`, icon: Clock, color: "amber", trend: "-5%", trendUp: false },
//           { label: "Overdue Amount", value: `$${(stats.overdueAmount / 1000).toFixed(0)}K`, icon: AlertCircle, color: "red", trend: "+12%", trendUp: false },
//           { label: "Total Transactions", value: stats.totalTransactions, icon: CreditCard, color: "indigo", trend: "+8%", trendUp: true },
//         ].map((stat, idx) => (
...
//               </div>
//             </div>
//             <div className={`flex items-center gap-1 mt-2 text-xs ${stat.trendUp ? "text-emerald-400" : "text-red-400"}`}>
...
//         {[
//           { label: "Paid", value: stats.paidCount, color: "emerald", icon: CheckCircle },
//           { label: "Pending", value: stats.pendingCount, color: "amber", icon: Clock },
//           { label: "Overdue", value: stats.overdueCount, color: "red", icon: AlertCircle },
//         ].map((status, idx) => (
//           <div key={idx} className="relative bg-white/5 backdrop-blur-xl rounded-xl p-4 border border-white/10">
//             <div className="flex items-center gap-3">
//               <div className={`p-2 bg-${status.color}-500/20 rounded-lg border border-${status.color}-500/30`}>
...
//               <tr>
//                 <th className="text-left p-4 text-xs font-medium text-gray-400 uppercase tracking-wider">ID</th>
...
//                 <th className="text-left p-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Payment Method</th>
...
//                   <tr key={transaction.id} className="border-b border-white/5 hover:bg-white/5 transition-all duration-300">
//                     <td className="p-4">
//                       <p className="text-sm font-mono text-white">{transaction.id}</p>
//                     </td>
//                     <td className="p-4">
//                       <div>
//                         <p className="text-sm font-medium text-white">{transaction.customer.name}</p>
//                         <p className="text-xs text-gray-400 capitalize">{transaction.customer.type}</p>
//                       </div>
//                     </td>
//                     <td className="p-4">
//                       <div className="flex items-center gap-2">
...
//                     </td>
//                     <td className="p-4">
//                       <p className="text-sm font-semibold text-white">${transaction.amount.toLocaleString()}</p>
//                     </td>
//                     <td className="p-4">
...
//                         <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPaymentMethodBadge(transaction.paymentMethod)}`}>
...
//                           </button>
//                         )}
//                         <button className="p-1.5 hover:bg-white/10 rounded-lg transition-all" title="Download">
...
//             {transactions.filter(t => t.status === "paid" || t.status === "completed").slice(0, 3).map((payment) => (
//               <div key={payment.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
//                 <div>
//                   <p className="text-sm font-medium text-white">{payment.customer.name}</p>
...
//                   <p className="text-xs text-emerald-400">{payment.paymentMethod?.replace("_", " ").toUpperCase()}</p>
//                 </div>
//               </div>
//             ))}
//           </div>
...
//           </div>
//           <div className="relative p-5 space-y-3">
//             {transactions.filter(t => t.status === "pending" || t.status === "overdue").slice(0, 3).map((invoice) => (
//               <div key={invoice.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
//                 <div>
//                   <p className="text-sm font-medium text-white">{invoice.customer.name}</p>
//                   <p className="text-xs text-red-400">Due: {invoice.dueDate}</p>
//                 </div>
//                 <div className="text-right">
//                   <p className="text-sm font-bold text-white">${invoice.amount.toLocaleString()}</p>
//                   <button className="text-xs text-indigo-400 hover:text-indigo-300">Send Reminder</button>
//                 </div>
//               </div>
//             ))}
//           </div>
...
//           onSave={(newInvoice) => {
//             setTransactions([{ ...newInvoice, id: `INV-2024-${String(transactions.length + 1).padStart(3, '0')}` }, ...transactions]);
...
//                 <p className="text-sm text-white">{transaction.customer.name}</p>
//               </div>
//               <div>
//                 <p className="text-xs text-gray-400">Type</p>
//                 <p className="text-sm text-white capitalize">{transaction.customer.type}</p>
//               </div>
//               <div>
//                 <p className="text-xs text-gray-400">Email</p>
//                 <p className="text-sm text-white">{transaction.customer.email}</p>
//               </div>
//             </div>
//           </div>
...
//                 <span className="text-sm font-bold text-white">${transaction.amount.toLocaleString()}</span>
//               </div>
//               <div className="flex justify-between py-2 border-b border-white/10">
//                 <span className="text-sm text-gray-400">Payment Method</span>
//                 <span className="text-sm text-white capitalize">{transaction.paymentMethod?.replace("_", " ")}</span>
//               </div>
//               {transaction.dueDate && (
//                 <div className="flex justify-between py-2 border-b border-white/10">
//                   <span className="text-sm text-gray-400">Due Date</span>
//                   <span className="text-sm text-white">{transaction.dueDate}</span>
//                 </div>
//               )}
//               {transaction.paymentDate && (
//                 <div className="flex justify-between py-2">
//                   <span className="text-sm text-gray-400">Payment Date</span>
//                   <span className="text-sm text-white">{transaction.paymentDate}</span>
//                 </div>
//               )}
//               {transaction.transactionId && (
//                 <div className="flex justify-between py-2">
//                   <span className="text-sm text-gray-400">Transaction Reference</span>
//                   <span className="text-sm font-mono text-white">{transaction.transactionId}</span>
//                 </div>
//               )}
//             </div>
//           </div>
...
//                       <p className="text-xs text-gray-400">{item.quantity} x ${item.unitPrice.toLocaleString()}</p>
//                     </div>
...
//               </button>
//             )}
//             <button className="flex-1 px-4 py-2 border border-white/10 rounded-lg text-gray-300 hover:bg-white/10 transition-all flex items-center justify-center gap-2">
//               <Download className="w-4 h-4" />
//               Download PDF
//             </button>
//             <button className="flex-1 px-4 py-2 border border-white/10 rounded-lg text-gray-300 hover:bg-white/10 transition-all flex items-center justify-center gap-2">
...
//       items: [{ description: formData.description, quantity: 1, unitPrice: parseFloat(formData.amount), total: parseFloat(formData.amount) }],
//       paymentDate: null,
//       transactionId: null,
//     });
//   };
...
const PaymentsInvoiceManagement = () => {
  return (
    <div className='text-white'>Payments & Invoice Management</div>
  )
}

export default PaymentsInvoiceManagement