...
export default function NotificationsAnnouncements() {
  ...
  const typeOptions = [
    ...
    { value: "announcement", label: "Announcements", color: "blue", icon: Megaphone },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: "Total Notifications", value: stats.total, icon: Megaphone },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {audienceOptions.filter(a => a.count !== null).map((audience) => (
          <div key={audience.value} className="relative bg-white/5 backdrop-blur-xl rounded-2xl p-4 border border-white/10 hover:border-blue-500/30 transition-all duration-300">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/10 rounded-lg border border-white/20">
                <audience.icon className="w-4 h-4 text-blue-400" />
              </div>
              <div>
                    ...
                    <td className="p-4">
                      <div className="flex gap-1">
                        {notification.channel.map((ch) => {
                          ...
                          return channel ? (
                            <span key={ch} className={`px-1.5 py-0.5 text-[10px] font-medium rounded bg-blue-500/20 text-blue-400`}>
                              {channel.label}
                            </span>
                          ) : null;
                        })}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="relative bg-gradient-to-r from-blue-500/10 to-cyan-500/10 backdrop-blur-xl rounded-2xl p-4 border border-blue-500/30 hover:border-blue-500/50 transition-all duration-300">
          <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="flex-1 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-blue-500/50"
                placeholder="Add tag..."
              />
              <button
                type="button"
                onClick={handleAddTag}
                className="px-4 py-2 bg-white/10 rounded-xl text-gray-300 hover:bg-white/20 transition-all"
              >
                Add
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {formData.tags.map((tag) => (
                <span key={tag} className="inline-flex items-center gap-1 px-2 py-1 bg-blue-500/20 rounded-lg text-xs text-blue-400">
                  {tag}
