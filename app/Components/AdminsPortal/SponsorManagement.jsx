...
export default function SponsorManagement() {
  ...
  const [sponsors, setSponsors] = useState([
    ...
    },
    {
      ...
      status: "pending",
      joinDate: "2024-02-01",
      benefits: ["Standard Booth Location", "Logo on Website"],
  ...
  const tiers = [
    ...
    { value: "platinum", label: "Platinum", color: "blue", icon: Crown },
  ...
  const stats = {
    ...
    totalRevenue: sponsors.reduce(
      (sum, s) => sum + parseInt(s.amount.replace("$", "").replace(",", "")),
      0,
    ),
  ...
  const handleStatusToggle = (id, currentStatus) => {
    ...
    setSponsors(
      sponsors.map((s) => (s.id === id ? { ...s, status: newStatus } : s)),
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
        {[
          { label: "Total Sponsors", value: stats.total, icon: Briefcase },
          ...
          { label: "Platinum", value: stats.platinum, icon: Crown },
          { label: "Gold", value: stats.gold, icon: Medal },
          {
            label: "Silver/Bronze",
            value: stats.silver + stats.bronze,
            icon: Star,
          },
        ].map((stat, idx) => (
          <div
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {tiers
          ...
          .map((tier) => {
            ...
            const tierRevenue = tierSponsors.reduce(
              (sum, s) =>
                sum + parseInt(s.amount.replace("$", "").replace(",", "")),
              0,
            );
            return (
              <div
                key={tier.value}
                className="relative bg-white/5 backdrop-blur-xl rounded-2xl p-4 border border-white/10 hover:border-blue-500/30 transition-all duration-300 cursor-pointer"
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${getTierBadge(sponsor.tier)}`}
                        >
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusBadge(sponsor.status)}`}
                      >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() =>
                            handleStatusToggle(sponsor.id, sponsor.status)
                          }
                          className="p-1.5 hover:bg-white/10 rounded-lg transition-all"
                          title={
        ...
        <SponsorFormModal
          ...
          onSave={(sponsorData) => {
            if (showEditModal) {
              setSponsors(
                sponsors.map((s) =>
                  s.id === sponsorData.id ? sponsorData : s,
                ),
              );
            } else {
              setSponsors([
                {
                  ...
                  logo: sponsorData.companyName.substring(0, 2).toUpperCase(),
                },
                ...sponsors,
              ]);
            }
          ...
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/10">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`px-2 py-0.5 text-xs font-medium rounded-full ${getTierBadge(sponsor.tier)}`}
                >
                  {sponsor.tier.charAt(0).toUpperCase() + sponsor.tier.slice(1)}
                </span>
                <span
                  className={`px-2 py-0.5 text-xs font-medium rounded-full ${getStatusBadge(sponsor.status)}`}
                >
                  {sponsor.status}
                </span>
              </div>
          ...
          <div className="p-4 bg-white/5 rounded-xl border border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              {sponsor.benefits.map((benefit, idx) => (
                <span
                  key={idx}
                  className="px-2 py-1 bg-emerald-500/10 rounded-lg text-xs text-emerald-400 border border-emerald-500/30"
                >
                  {benefit}
                </span>
              ))}
...
const SponsorFormModal = ({
  ...
  const handlePackageChange = (pkgName) => {
    const selectedPkg = packages[formData.tier]?.find(
      (p) => p.name === pkgName,
    );
    setFormData({
      ...formData,
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      ...formData,
      joinDate: sponsor?.joinDate || new Date().toISOString().split("T")[0],
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-white/5 backdrop-blur-2xl rounded-2xl w-full max-w-3xl shadow-2xl border border-blue-500/20 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
        <div className="relative p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    yearsSponsored: parseInt(e.target.value),
                  })
                }
                className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-blue-500/50"
              />
