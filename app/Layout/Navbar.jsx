      ...
      <div className="max-w-[1300px] mx-auto px-5">
        <div className="flex items-center justify-between h-20" ref={dropdownRef}>
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            >
              Medconf
              <span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent ps-2">
                KSA
              </span>
            </span>
          </Link>
            ...
            {navlinks.map((link) => (
              <div key={link.label} className="relative">
                ...
                {link.dropdown && activeDropdown === link.label && (
                  <div className="absolute left-1/2 -translate-x-1/2 mt-3 w-64">
                    <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
                      <div className="py-2">
                        {link.dropdown.map((item) => (
                          <Link
                            key={item.label}
                            href={item.href}
                            className="block px-5 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-[#0066FF] transition-all"
                          >
                            {item.label}
                          </Link>
                        ))}
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <button className="bg-[#0066FF] hover:bg-[#0052CC] text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg">
              Sign In
            </button>
          </div>
